    ...
    stages {
        ...
        stage('CI Pipeline Stages') {
            ...
            stages {
                ...
                stage('Checkout') {
                    when {
                        expression {return env.CONTINUE_PIPELINE == 'true' || env.PIPELINE_TYPE == 'CI-Light' || return env.PIPELINE_TYPE == 'CD'  }
                    }
                    steps {
                ...
                stage('Post-Build') {
                    when {
                        expression { return env.CONTINUE_PIPELINE == 'true' && env.PIPELINE_TYPE == 'CI' || return env.PIPELINE_TYPE == 'CD'  }
                    }
                    steps {
                          script {
                                 // Archive artifacts 
